...
std::vector<Board::SquareCoords> Board::getWordSquares(Board::SquareCoords startSquare, size_t wordLength, bool horizontal) const
{
	...
	for(int letterIndex = 0; letterIndex < static_cast<int>(wordLength); ++letterIndex)
	{
		...
		if(horizontal)
		{
			//iterates to the next square
			++currCoords.col;
			//checks to make sure we do not go past the boundry on the board
		...
		{
			//itertes to the next square
			++currCoords.row;
			//checks to make sure we do not go past the boudry on the board
...
_isFirstMove(false)
{
	...
	{
		//if we are the first line in the boardFile
		if(lineIndex == 0)
		{
			boardFileStream >> _numColumns;
			boardFileStream >> _numRows;
		}
		//if we are on the second line, we can get the start positions
		else if(lineIndex == 1)
		{
		...
		//starts to actually "make" the board
		else
		{
			std::string row;
		...
		//increments to the next line (why we dont need 2 nested for loops)
		++lineIndex;
	}
}
...
{
	//looks at board and finds how the tiles will be placed
...
Square* Board::getSquareM(size_t x, size_t y) const {
 	return _squares.at(x-1).at(y-1);
}
...
void Board::initBoard(std::string init) {
	ifstream ifile(init);
	string temp;
	//iterates through the rows and columns and sets up the board with a possible initialization file
	for (size_t i =0;i < _numRows;i++) {
		//clears out temp and then brings in the next line from the file
		temp.clear();
		getline(ifile,temp);

		for (size_t j = 0; j< _numColumns; j++) {
			//brings in three letters at a time into the tempLine string
			string tempLine = "";
			tempLine += temp[j*3];
			tempLine += temp[j*3+1];
			tempLine += temp[j*3+2];
			//if the line is not equal to "..." we change the board
			if (tempLine != "...") {
				int currScore = (((int)tempLine[1]-'0')*10)+((int)tempLine[2]-'0');
				
				_squares[i][j]->placeTile(new Tile(tempLine[0],currScore));
			}
		}
	}
}